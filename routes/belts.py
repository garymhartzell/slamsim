...
# --- NEW: Belt History Routes ---

@belts_bp.route('/history/<string:belt_id>')
def history(belt_id):
    """Displays the championship history for a specific belt."""
    ...
    for reign in history:
        try:
            date_won = datetime.strptime(reign['Date_Won'], date_format).date()
            if reign.get('Date_Lost'):
                date_lost = datetime.strptime(reign['Date_Lost'], date_format).date()
                reign['Days'] = (date_lost - date_won).days
            else:
                reign['Days'] = (today - date_won).days # Calculate for active reign
        except (ValueError, TypeError):
            reign['Days'] = 'Error' # Handle invalid date format

    # Sort history by Date_Won descending
    history.sort(key=lambda r: r.get('Date_Won', '0'), reverse=True)
    
    return render_template('belts/history.html', belt=belt, history=history)

def _get_reign_form_data(form):
    """Extracts reign data from the history form."""
    return {
        'Belt_ID': form.get('belt_id'),
        'Champion_Name': form.get('champion_name'),
        'Date_Won': form.get('date_won'),
        'Date_Lost': form.get('date_lost') or None, # Store as None if empty
        'Defenses': form.get('defenses', 0, type=int),
        'Notes': form.get('notes', '').strip()
    }

@belts_bp.route('/history/<string:belt_id>/add_reign', methods=['GET', 'POST'])
def add_reign(belt_id):
    """Handles adding a new reign to a belt's history."""
    belt = get_belt_by_id(belt_id)
    if not belt:
        flash('Belt not found.', 'danger')
        return redirect(url_for('belts.list_belts'))
...
def edit_reign(reign_id):
    """Handles editing an existing reign."""
    ...
    if request.method == 'POST':
        ...
        updated_data['Reign_ID'] = reign_id # Ensure Reign_ID is preserved
        success, message = update_reign_in_history(reign_id, updated_data)
        flash(message, 'success' if success else 'danger')
        if success:
            return redirect(url_for('belts.history', belt_id=belt['ID']))
