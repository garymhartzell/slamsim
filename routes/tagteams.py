...
def _get_form_data(form):
    ...
    data = {
        "Name": escape(form.get('Name', '')).strip(), "Wins": escape(form.get('Wins', '0')).strip(),
        "Losses": escape(form.get('Losses', '0')).strip(), "Draws": escape(form.get('Draws', '0')).strip(),
        "Status": tagteam_status, "Division": escape(form.get('Division', '')).strip(),
        "Location": escape(form.get('Location', '')).strip(), "Weight": escape(form.get('Weight', '')).strip(),
        "Alignment": form.get('Alignment', ''), "Music": escape(form.get('Music', '')).strip(),
        "Members": members_string, "Faction": escape(form.get('Faction', '')).strip(),
        "Manager": escape(form.get('Manager', '')).strip(), "Moves": escape(form.get('Moves', '')).strip(),
        "Awards": escape(form.get('Awards', '')).strip()
    }
    if 'Name' in form:
        existing_team = get_tagteam_by_name(form['Name'])
        if existing_team: data['Belt'] = existing_team.get('Belt', '')
    return data
...
def create_tagteam():
    ...
    if request.method == 'POST':
        ...
        tagteam_data['Belt'] = ''
        if not tagteam_data['Name']: flash('Tag-team Name is required.', 'danger')
        elif not request.form.get('Member1') or not request.form.get('Member2'): flash('At least two members are required.', 'danger')
        elif get_tagteam_by_name(tagteam_data['Name']): flash(f"A tag-team with the name '{tagteam_data['Name']}' already exists.", 'danger')
        else:
...
def edit_tagteam(tagteam_name):
    ...
    if request.method == 'POST':
        ...
        updated_data['Belt'] = tagteam.get('Belt', '')
        if not updated_data['Name']: flash('Tag-team Name is required.', 'danger')
        elif not request.form.get('Member1') or not request.form.get('Member2'): flash('At least two members are required.', 'danger')
        elif updated_data['Name'] != tagteam_name and get_tagteam_by_name(updated_data['Name']): flash(f"A tag-team with the name '{updated_data['Name']}' already exists.", 'danger')
        else:
